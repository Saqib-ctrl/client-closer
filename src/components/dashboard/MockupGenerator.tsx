...
interface MockupUsage {
  used: number;
  limit: number;
  isPremium: boolean;
}

export const MockupGenerator = ({ userId, userEmail, onMockupSaved }: MockupGeneratorProps) => {
  const [uploadedImages, setUploadedImages] = useState<string[]>([]);
  const [stylePrompt, setStylePrompt] = useState("");
  const [isGenerating, setIsGenerating] = useState(false);
  const [isSaving, setIsSaving] = useState(false);
  const [generatedImage, setGeneratedImage] = useState<string | null>(null);
  const [usage, setUsage] = useState<MockupUsage>({ used: 0, limit: 5, isPremium: false });
  ...
  const fetchUsage = async () => {
    const { data } = await supabase
      .from("user_usage")
      .select("mockups_generated, mockups_limit, is_premium")
      .eq("user_id", userId)
      .single();

    if (data) {
      setUsage({
        used: (data as any).mockups_generated ?? 0,
        limit: (data as any).mockups_limit ?? 5,
        isPremium: data.is_premium ?? false,
      });
    }
  };

  const isLimitReached = !usage.isPremium && usage.used >= usage.limit;
  ...
  const generateMockup = async () => {
    ...
    if (isLimitReached) {
      toast({
        title: "Limit reached",
        description: "Upgrade to Pro for unlimited mockups.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (!session?.access_token) {
        throw new Error("Please log in to generate mockups.");
      }
        ...
        <div className="space-y-6">
          <div className="space-y-3">
            ...
            >
              ...
              <p className="font-medium mb-1">
                {isLimitReached ? "Upgrade to continue" : "Click to upload images"}
              </p>
          ...
          <div className="flex gap-3">
            <Button
              ...
              disabled={isGenerating || uploadedImages.length === 0 || isLimitReached}
              className="flex-1"
            >
              {isGenerating ? (
                <>
        ...
        <div className="space-y-6">
          ...
          {generatedImage && !isGenerating && (
            <div className="rounded-lg border border-border bg-card overflow-hidden">
              ...
              <div className="p-4">
                <img src={generatedImage} alt="Generated mockup" className="w-full rounded-lg" />
              </div>
            </div>
          )}
